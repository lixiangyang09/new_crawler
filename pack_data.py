...
def pack_report_data():
    ...
    packed_files = [file_name for file_name in os.listdir(report_data_dir) if file_name.endswith(constants.data_file_suffix)]
    ...
    data_folders = [folder for folder in os.listdir(output_base_dir) if os.path.isdir(folder)]
    for data_folder in data_folders:
        if data_folder not in packed_files_date:
            ...
            if os.path.exists(seeds_file):
                ...
                FileService.pack_folders(pack_file_name, [report_data_dir + '/' + data_folder])
                FileService.pack_files(pack_file_name, [seeds_file])
